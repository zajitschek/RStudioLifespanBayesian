...
data1 <- data1 %>% mutate(across(where(is.integer), as.factor), status = 1)  # deaths were observed for all individuals

# Your first Bayesian Cox PH model
#  For now, we will assume that Gaussian errors are ok (this is the default)

stan_surv_model1 <- stan_surv(Surv(lifespan, status) ~ assaydiet + (1|vial), data= data1, seed= 111)
